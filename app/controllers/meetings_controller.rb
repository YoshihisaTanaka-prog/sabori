class MeetingsController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      format.html {
        ...
        @meetings = Meeting.where(eventId: min..max).order(:id)
      ...
      format.xlsx {
        ...
        max = current_user.start_number + 98
        @meetings = Meeting.where(eventId: min..max).order(:id)
        @programs = Program.where(event_id: min..max).order(:id)
        @performers = Performer.where(event_id: min..max).order(:id)
  ...
  def show
    @programs = Program.where(event_id: @meeting.eventId).order(:id)
  end
